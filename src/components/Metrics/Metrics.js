...
export default class Metrics extends React.Component {
    ...
    if (this.state.question.id == -1) {
      ...
      if (this.state.quiz.id == -1) {
          ...
          if (this.state.section.id == -1) {
              ...
              var quizIDArray = [];
              Api.db.find('studentanswer', {section: section.id})
        ...
        for(var i = 0; i < student_answer_sorted.length; i++) {
          ...
          if (currentQuizId != student_answer_sorted[i].quiz.id) {
            ...
            Promise.each(quizIDArray, function(quiz) {
              return Api.db.find('question', {quiz: quiz, type: type})
            ...
            var totalStudents;
              ...
              Api.db.post('student/getStudentsBySectionId/', {id: section.id})
  ...
  //BOTTOM
              ...
              var quizAnswerCorrectArray = [];
              var quizIDArray = [];
              Api.db.find('studentanswer', {section: selected_section.id})
              .then(function(student_answer){
                console.log("student_answer: ", student_answer);
        ...
        for(var i = 0; i < student_answer_sorted.length; i++) {
          ...
          if (currentQuizId != student_answer_sorted[i].quiz.id) {
            ...
            var totalStudents;
              //Get total number of students
              Api.db.post('student/getStudentsBySectionId/', {id: selected_section.id})
              .then(function(students){
                  totalStudents = students.length;
                  console.log("totalStudents: ", totalStudents);
  ...
  //BOTTOM
            ...
            Promise.each(questions, function(question) {
              ...
              me.getAnswers(question,function(answers){
                ...
                Promise.each(answers, function(answer) {
                  ...
                  return Api.db.find('studentanswer', holder)
        ...
        this.getAnswers(selected_question,function(answers){
          ...
          var data = {};
          Promise.each(answers, function(answer) {
            return Api.db.post('studentanswer/getStudentCountByAnswerId/', {id: answer.id,section: section_id})
            .then(function(count){
              counts.push(count);
            });
  ...
  Api.db.find('studentanswer', {question: selected_question.id})
    ...
    Api.db.find('studentanswer', {section: studentSection.id})
    ...
    if (section_sorted_student_answers.length > 0) {
        ...
        for(var g = 0; g < section_sorted_student_answers.length; g++) {
          ...
          Promise.each(quizSectionIDArray, function(quiz_section) {
            return Api.db.find('question', {quiz: quiz_section, type: type_section})
            ...
            var totalStudents;
              //Get total number of students
              Api.db.post('student/getStudentsBySectionId/', {id: studentSection.id})
...
//AVERAGE DATA!!!!!!!!
    ...
    var quizAnswerCorrectArray = [];
    var quizIDArray = [];
    Api.db.find('studentanswer', {student: selected_student.id})
      .then(function(student_answer){
        console.log("student_answer: ", student_answer);
      ...
      // Promise.each(quizzes, function(quiz){
        ...
        .then(function(questions){
          ...
          if(questionCount == questions.length - 1) {
            ...
            var resultsCorrect = [];
            var found = 0;
            Api.db.find('studentanswer',{student: selected_student.id, quiz: selected_quiz.id})
  ...
  if (selected_question.type == "multipleChoice"){
  Api.db.find('studentanswer', {student: selected_student.id, quiz: selected_quiz.id, question: selected_question.id})
      ...
      // console.log("answer[answer]", answer[answer1]);
        return answer[0];
            ...
            console.log("studentAnswered: ", studentAnswered);
                //find other answers
            var answerValues = [];
            Api.db.find('answer', {question: answer_return.question.id})
  ...
  Api.db.find('studentanswer', {question: selected_question.id, student: selected_student.id})
    .then(function(student_answers){
      console.log("student_answer: ", student_answers);
      // console.log("student_answer.text: ", student_answers[0].text);
      var complete_text = "<h2>Student Answers:</h2> <hr COLOR='grey' SIZE='2'>";
...
createSectionMetric(secTitles, quizTitlesArrays, quizPercentsArrays){
  ...
  getAnswers(selected_question,res) {
    Api.db.find('answer', {question: selected_question.id})
